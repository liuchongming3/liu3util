...
/**
 * 一个用itextpdf生成pdf文件的示例
 * @Author: liutianshuo
 * @Date: 2025/7/1
 */
public class ItextpdfUtil {

    public static Font FONT_NORMAL = new Font(getBaseFont(), 12, Font.UNDEFINED);
    public static Font FONT_BOLD = new Font(getBaseFont(), 12, Font.BOLD);
    ...
    public Document generateItextPdfDocument(OutputStream os) throws Exception {
        ...
        for (int i = 0; i < userList.size(); i++) {
            table.addCell(createCell(userList.get(i).id + ""));
            table.addCell(createCell(userList.get(i).userName));
            table.addCell(createCell(userList.get(i).email));
            table.addCell(createCell(userList.get(i).phoneNumber + ""));
            table.addCell(createCell(userList.get(i).description));
        }
        document.add(table);
    ...
    private List<User> getUserList() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            user.userName = "pdai" + i;
            user.email = "pdai@pdai.tech" + i;
            user.phoneNumber = "121231231231L";
            userList.add(user);
        }
        return userList;
    }
